...
We can plot `GBIC` values across the tuning values. 
...
We can ultimately fit the model with the optimal tuning parameter (here, 0.114),
that is, the one generating the penalized model with the lowest GBIC. The
`summary` method details information on the model characteristics, the
...
Fisher information matrix (Geminiani et al., 2021). The last
columns report 95% confidence intervals (CI) for the model parameters. Standard
errors and CI of the penalized parameters shrunken to zero are not displayed. A
different significance level can be specified through the `level` argument in
the `summary` call.
...
q <- plotly::plot_ly(x = eta.grid, y = GBIC.mcp, type = 'scatter', mode = 'lines')
plotly::layout(q,  xaxis = list(showline = TRUE),
               title = list(text = "GBIC values across tuning parameters"))
```
